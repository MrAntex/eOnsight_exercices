...
class file_info:
	...
	def copy_into( self, t_fh, s_band = 1, t_band = 1, nodata_arg=None ):
		...
		bs = s_fh.GetRasterBand(s_band).GetBlockSize()
		result = 0
		for x in range(0, sw_xsize / bs[0]):
			dx = min(bs[0], sw_xsize - x*bs[0])
			if dx > 0:
				for y in range(0, sw_ysize / bs[1]):
					dy =  min(bs[1], sw_ysize - y*bs[1])
					if dy > 0:
						result += \
							raster_copy( s_fh, sw_xoff + x*bs[0], sw_yoff + y*bs[0], dx, dy, s_band,
									t_fh, tw_xoff + x*bs[0], tw_yoff + y*bs[0], dx, dy, t_band,
									nodata_arg )
		return result
#		return \
